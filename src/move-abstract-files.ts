...
export function isValidMoveTarget(
  ...
  return (
    ...
    !(file instanceof Obsidian.TFolder && destination.path.startsWith(file.path + "/"))
  );
}
...
function pathExists(app: Obsidian.App, path: string): boolean {
  ...
  const file =
    vault.getAbstractFileByPathInsensitive?.(path) ?? app.vault.getAbstractFileByPath(path);
  return file !== null;
}
